      ...
      <div class="modal-content">
        ...
        <div v-if="!twoFactorEnabled" class="modal-body">
          ...
          <div class="d-flex justify-content-between">
            ...
            >
              <div class="">Enable 2fa</div>
            </button>
          </div>
        </div>
        <div v-else class="modal-body d-flex justify-content-between">
          <button @click="disable2fa()" class="btn btn-danger w-100">
            Disable 2fa
          </button>
        </div>
      </div>
...
async function enable2fa() {
  ...
  const r = await sendBackendRequest("/auth/enable-2fa", {
    method: "POST",
    body: JSON.stringify({ "2fa_code": code.value }),
    headers: {
